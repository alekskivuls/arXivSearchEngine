...
DiskPositionalIndex::DiskPositionalIndex(const fs::path &path) : mPath(path) {
	mVocabList.open(fs::path(mPath).append("/vocabList.bin").string(), 
		std::ios_base::in | std::ios_base::binary);
   mPostings.open(fs::path(mPath).append("/postings.bin").string(),
		std::ios_base::in | std::ios_base::binary);
...
vector<VocabEntry> DiskPositionalIndex::ReadVocabTable(const fs::path & path) {
...
VocabEntry DiskPositionalIndex::BinarySearchVocabulary(const icu::UnicodeString &term) const {
...
vector<DiskPosting> DiskPositionalIndex::ReadPostingsFromFile(std::ifstream &postings, uint64_t postingsPosition) {
...
std::string DiskPositionalIndex::ReadVocabStringAtPosition(size_t i) const {
...
vector<DiskPosting> DiskPositionalIndex::GetPostings(const icu::UnicodeString &term) const {
